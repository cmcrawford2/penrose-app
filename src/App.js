    ...
    return (
      <div className="App">
        ...
        <div className="About-container">
          ...
          <p>Roger Penrose discovered a way to tile the infinite plane with pentagonal symmetry. The tiling can be expanded indefinitely, but it never repeats. The <a href="https://en.wikipedia.org/wiki/Penrose_tiling">Wikipedia article on Penrose tiles</a> is a good place to start to learn more. In my version, two diamonds are fitted together according to certain rules. These rules only allow eight different vertex configurations. After a tile is added, the code traverses the perimeter of the tiling, using a lookup table to find vertices that demand the addition of a specific tile. You can see how this works by disabling auto-fill and traversing the boundary one tile at a time.</p>
          <p>One of the properties of this way of tiling is that legal local modifications may create an illegal tiling at a distant location. You can see this effect by choosing to add a narrow tile to the default white edge six times in a row. Then toggle auto-fill and force the program to traverse the perimeter until the flawed tiling is complete.</p>
